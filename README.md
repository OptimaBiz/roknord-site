...
`npm run deploy:timeweb` по явному запросу публикует PHP-кабинет и статический сайт на Timeweb через SSH. Проверяет PHP, инициализирует схему без демо и проверяет HTTPS API до и после загрузки `dist/`. При ошибке возвращает ненулевой код. Настройки доступа и PHP: [.github/TIMEWEB_DEPLOYMENT.md](.github/TIMEWEB_DEPLOYMENT.md). Push в main сам по себе сайт не публикует. `docs/` и ZIP-архив — старые публикационные артефакты, не исходники.
